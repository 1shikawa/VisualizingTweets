...
class Index(TemplateView):
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        with concurrent.futures.ThreadPoolExecutor(
            max_workers=3, thread_name_prefix="thread"
        ) as executor:
...
def scraping_yahoo_news() -> pd.DataFrame:
    ...
    Returns:
        pd.DataFrame: YahooニュースDataFrameを返却
    """
...
def get_twitter_trend_df(parent_id: str) -> pd.DataFrame:
    ...
    for trend in trends[0]["trends"]:
        se = pd.Series(
            [trend["name"], trend["tweet_volume"], trend["url"]], twitter_trend_columns
        )
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        try:
            if screen_name and display_number:
                ...
                # created_atをキーに昇順ソート
                sorted_df_created_at = tweets_df.sort_values(
...
class FollowUsers(TemplateView):
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        try:
            ...
            # フォロワー数が多い順にソート
            follow_users_df = follow_users_df.sort_values(
...
class StockAdd(LoginRequiredMixin, CreateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if Stock.objects.filter(
            tweet_id=tweet_id, stock_user=str(self.request.user)
        ).exists():
        ...
        try:
            ...
            if Stock.objects.filter(
                ...
                tweet = api.get_status(
                    id=tweet_id, result_type="mixed", tweet_mode="extended"
                )
                if tweet.entities["urls"]:
                    expanded_url = tweet.entities["urls"][0]["expanded_url"]
                else:
